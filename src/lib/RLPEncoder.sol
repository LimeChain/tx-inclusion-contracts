...
library RLPEncoder {
    ...
    function encodeCallData(bytes memory data, uint256 offset) internal pure returns (bytes memory) {
        ...
        return abi.encodePacked(buffer.data);
    }

    function encodeBytesBuffer(bytes memory data) internal pure returns (bytes memory) {
        DynamicBufferLib.DynamicBuffer memory buffer;
        buffer.append(RLPWriter.writeBytes(data));
        return abi.encodePacked(buffer.data);
    }
